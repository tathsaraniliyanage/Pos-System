...
async function searchItems(query) {
    ...
    try {
        const item = await get_items_byCode(query);
        console.log(item); // This will correctly log the item after the AJAX request is resolved
    ...
    //    return await get_items_byCode(query);
...
$('#home-searchInput').on('keyup', async function (event) {


    if ('Enter' === event.key) {
        var item_code = $('#home-searchInput').data('ic');
        // let foundItem = item.find(element => element.code === item_code);
        // let foundItem = await get_items_byCode(query);

        try {
            const foundItem = await get_items_byCode(query);
            // console.log(foundItem); // This will correctly log the item after the AJAX request is resolved
        ...
        if (query === "") {
            ...
            try {
                ...
                tem_order.push(results)

                $('#home-search-name').text(results.item_name)
                $('#home-search-price').text(results.price)
                $('#home-search-description').text(results.description)
                $('#home-search-img').attr('src', results.image);
                $('#home-searchInput').attr('data-ic', results.code);
                ...
                const itemHtml = `<div  class="body rounded-3 shadow d-flex flex-wrap p-3 justify-content-start gap-2 flex-column"
                        ...
                        <div class="d-flex flex-column">
                            <span style="font-weight: 500; font-size: 20px;color: #464646; padding: 0%; margin: 0;">${results.item_name}</span>
                            <span style="font-weight: 400; font-size: 13px;color: #464646;">${results.description}</span>
                        </div>
                        <div class="d-flex flex-wrap justify-content-center align-items-center" style="height: 175px;position: relative;">
                        <img alt="" src=${results.image} height="128px">
                        </div>
                        

                        <div  class="d-flex flex-wrap justify-content-center gap-3 w-100">
                            <button data-ic="${results.code}" onclick="decrees(this)" class="rounded-3 border-0 text-white fw-bold d-flex align-items-center btn-qty">
                                <span style="margin: 0;padding: 0;">-</span>
                            </button>
                            <span id="${results.code}-qty" style="margin: 0; padding: 0;">${results.qty === 0 ? 0 : 1}</span>
                            <button data-ic="${results.code}" onclick="increase(this)" class="rounded-3 border-0 text-white fw-bold d-flex align-items-center btn-qty">
                                <span style="margin: 0;padding: 0;">+</span>
                            </button>
                        </div>


                        <button data-ic="${results.code}" id="${results.code}-item-button" class="add-to-cart-btn border-0" onclick="loadOrder(this)">

                            <span style="width: fit-content;">LKR ${results.price}</span>
                ...
                let foundItem = tem_order.find(element => element.code === results.code);
                if (foundItem != undefined) {
                    $(`#${item.code}-item-button`).css({ 'background-color': '#60DD94' });
                }
